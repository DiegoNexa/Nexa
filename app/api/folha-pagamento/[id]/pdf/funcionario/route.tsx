...
import { FolhaPdfFuncionario } from "@/components/folha-pagamento/folha-pdf-funcionario";

// Força Node runtime — @react-pdf/renderer não roda em Edge
...
export async function GET(request: Request, { params }: RouteContext) {
  ...
  // Auth guard explícito (proxy middleware já garante mas é boa prática)
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return new Response("Não autenticado", { status: 401 });

  // Nome do salão para o cabeçalho
  ...
  const buffer = await renderToBuffer(
    <FolhaPdfFuncionario nomeSalao={salaoNome} folha={folha} />,
  );

  const safeName = folha.profissional.nome.replace(/[^a-z0-9]+/gi, "-").toLowerCase();
  const filename = `recibo-${safeName}-${periodo.inicio.slice(0, 7)}.pdf`;
