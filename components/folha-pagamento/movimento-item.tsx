  ...
  return (
    <div className="flex items-center gap-3 p-3 rounded-xl" style={{ background: "rgba(255,255,255,0.03)" }}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {m.descricao && (
          <p className="text-xs text-on-surface-variant truncate mt-0.5">{m.descricao}</p>
        )}
      </div>
      <p className="text-sm font-semibold whitespace-nowrap" style={{ color: cor }}>
        {sinal} {BRL.format(Number(m.valor))}
      </p>
      <form action={excluirMovimento.bind(null, m.id, profissionalId)}>
        <button
          type="submit"
          aria-label="Excluir"
          onClick={(e) => {
